...
function Card({
  ...
  const isCard = () => {
    if (filterBuscet.find((i) => i.imageUrl === imageUrl)) {
      return true;
    }
  };

  const isFavorites = () => {
    if (isFavorite.find((i) => i.imageUrl === imageUrl)) {
      return true;
    }
  };
  ...
  const btnClickFavorite = () => {
    if (setFavoriteButton === undefined) {
      setIsLiked(!isLiked);
  ...
  return (
    <div className={styles.card}>
      <button
        className={
          isFavorites()
            ? `${styles.addFavourites} ${styles.addFavouritesAll}`
            : favoriteButton
            ? `${styles.addFavourites} ${styles.addFavouritesAll}`
            : isLiked
            ? `${styles.addFavourites} ${styles.addFavouritesAll}`
            : styles.addFavourites
        }
        onClick={btnClickFavorite}
      >
        {isFavorites() ? (
          <svg
            width="15"
      ...
      <div className={styles.wrapSumm}>
        ...
        <button
          className={
            isCard()
              ? `${stylesBtnAdd.addBasketAdd} ${stylesBtnAdd.addBasket}`
              : isAdd
              ? `${stylesBtnAdd.addBasketAdd} ${stylesBtnAdd.addBasket}`
              : stylesBtnAdd.addBasket
          }
